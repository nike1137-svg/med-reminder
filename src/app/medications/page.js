  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900">약 목록</h1>
        <Link
          href="/medications/new"
          className="rounded-lg bg-teal-600 px-4 py-2.5 text-base font-semibold text-white hover:bg-teal-700"
        >
          + 약 등록
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center text-base text-slate-500">
          등록된 약이 없습니다.
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-none flex-col gap-1.5 sm:flex-row">
                  <Link
                    ...
                    className="rounded-md px-3 py-2 text-center text-sm font-medium text-slate-500 hover:bg-slate-100"
                  >
                    수정
                  </Link>
                  <button
                    onClick={() => handleDelete(med.id, med.name)}
                    className="rounded-md px-3 py-2 text-center text-sm font-medium text-red-500 hover:bg-red-50"
                  >
                    삭제
                  </button>
                </div>
