...
- **배포 URL**: https://med-reminder-chi.vercel.app
- **로그인**: 이메일·비밀번호 방식입니다. 첫 방문이면 **"회원가입"**을 눌러 아무 이메일 + 비밀번호(6자 이상)를 입력하면 **확인 메일 없이 즉시 로그인**됩니다. 이후 같은 정보로 로그인하세요. (교육용 데모라 테스트 계정으로 확인하시면 됩니다)
- **핵심 흐름**: 로그인 → 약 등록 → "오늘" 화면에서 시간대별 체크. **약 등록/조회/수정/삭제·복약 체크·로그인은 항상 정상 동작**합니다.
- **AI 자동완성(베타)**: 약 등록 화면의 "자연어로 빠르게 입력"은 별도 AI 서버(운영자 PC)를 연결해 제공하므로 **운영자 PC 가동 시간에만** 동작합니다. 꺼져 있으면 안내 메시지가 뜨며, 수동 입력으로 정상 이용 가능합니다. (자세한 사용법·구조는 아래 [AI 자동완성](#ai-자동완성베타--사용법--구조) 참고)
